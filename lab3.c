...
Quanto maior o valor da dimensão mais o valor resultante se aproxima de pi. O valor resultante para o código sequencial e o
concorrente é o memso, pois o vetor é igual e a base do cálculo é a mesma. O que diferencia das duas formas é o tempo, quanto
maior a dimensão do vetor mais eficiente o código multithreads, em relação ao código sequencial, se demonstra.
Quando inicia com dimensão baixa o tempo sequencial chega a ser melhor, pois no trecho de código ele só realiza o cálculo,
enquanto o código multithreads realiza alocação, verificação, for para criação de threads e para esperar essas threads terminarem.
Então não se tem muita vantagem em usar o multithreads quando a dimensão é baixa.
